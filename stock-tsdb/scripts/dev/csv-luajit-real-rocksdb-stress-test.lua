...
-- CSV->LuaJIT->真实RocksDB数据流压力测试脚本
-- 使用真实的RocksDB数据库，而不是模拟存储
...
local config = {
    ...
    database_path = "/tmp/test_rocksdb_stress_db"
}
...
-- 主测试函数
local function main()
    print("=== CSV->LuaJIT->真实RocksDB数据流压力测试 ===")
    ...
    db = RocksDBFFI.get_library().rocksdb_open(options, config.database_path, errptr)
    
    if errptr[0] ~= nil then
        err = ffi.string(errptr[0])
        RocksDBFFI.get_library().rocksdb_free(errptr[0])
        print("❌ 打开RocksDB数据库失败:", err)
        return
    end
    ...
    -- 关闭数据库
    if db then
        RocksDBFFI.get_library().rocksdb_close(db)
        print("✅ RocksDB数据库已关闭")
    end
    ...
    -- 保存测试结果
    local results_file = "/tmp/csv_luajit_real_rocksdb_stress_test_results.json"
