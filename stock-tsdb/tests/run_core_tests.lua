...
local tests = {
    ...
    {name = "股票示例测试", script = "stock_example.lua"},
    {name = "集成TSDB测试", script = "integrated_tsdb_test.lua"},
    {name = "真实股票数据测试", script = "real_stock_data_test.lua"},
    {name = "集群基础测试", script = "tsdb_cluster_test.lua"}
}
...
for i, test in ipairs(tests) do
    ...
    if handle then
        ...
        if output and (string.find(output, "测试完成") or string.find(output, "完成") or 
                      string.find(output, "✓") or string.find(output, "成功") or
                      string.find(output, "===.*完成")) then
            success = true
        end
